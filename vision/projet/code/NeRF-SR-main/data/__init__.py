...
def find_dataset_using_name(dataset_name):
    ...
    """
    # importer le module name llff_dataset_dataset
    dataset_filename = "data." + dataset_name + "_dataset"
    datasetlib = importlib.import_module(dataset_filename)
# chercher la classe qui hrite baseDataset
    dataset = None
...
def create_dataset(opt, mode, shuffle=True):
    ...
    """
    #  initialiser un objet custom data set data loder
...
class CustomDatasetDataLoader():
    ...
    def __init__(self, opt, mode, shuffle):
        ...
        self.opt = opt
        self.mode = mode
        #  1 er desc choese affecter au data loeder nombre of batch per processus 
        if mode == 'train' or self.mode == 'train_crop':
            if opt.accelerator == 'ddp':
                #  assurer que nombre of batches est diviisble par coutn gpus 
                assert opt.batch_size % opt.n_gpus == 0
                # initaliser self.batch cad objet custom data set 
                self.batch_size = opt.batch_size // opt.n_gpus
                if opt.is_master:
                    print(f'Using DDP, batch size per proc = {self.batch_size} ({opt.batch_size}/{opt.n_gpus})')
            else:
                #  sinon notre le batch est executer sur un seul gpu 
                self.batch_size = opt.batch_size
        else:
            # si c ne pas un train donc batch de evalusation est recuperer 
            # dans evalusatiojn ya pas de multi gpu
            self.batch_size = opt.eval_batch_size
        # data set mode est type de data set qui llff downX donw sample
