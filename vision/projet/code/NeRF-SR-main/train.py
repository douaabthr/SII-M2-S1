...
def main(rank):
    # 1- TrainOptions() → crée une instance de la classe. cette classe herite de la class BaseOptions
    # ce objet contient tout les argument quil va  le lire  de ligne de commande 
    # .parse(rank) → exécute la méthode parse définie dans BaseOptions qui :

    # Lit les arguments passés en ligne de commande.

    # Remplit les valeurs par défaut si elles ne sont pas spécifiées.

    # Retourne un objet opt contenant toutes les options configurées, par ex. :
    opt = TrainOptions().parse(rank)   # get training options
    setup_env(opt)
    #  opt.trainsplit par defaut = train 
...
def run_ddp():
    ...
    # retourne le nombre de GPU disponibles sur la machine.
    # lancer plusieur processus chaqun'un dans un gpu  appeler main par leur rank
    mp.spawn(main, nprocs=n_gpus, join=True)

# done 
# cette condition est executer a chaque fois on execute ce fichier non pas une fonction dans ce fichier ...
if __name__ == '__main__':
    # argparse.ArgumentParser() cree un objet qui va lire les argument passe on ligne de commande 
    # on fait apple a la fonction add_distributed par objet qui ajoute  l'argument accelerator au objet praser pour le lire par la suite 
    parser = add_dist_options(argparse.ArgumentParser())
    #  cette fonction lire seulement les arg connu on bien definir dans l'objet parser dans ce cas accelerator 
    # et le metter dans la liste des option 
    opt, _ = parser.parse_known_args()
    #  verfier le type de l'accelerator 
