...
class LLFFDataset():
    ...
    def read_meta(self):
        ...
        camdata = read_cameras_binary('C:/Users/USER/Desktop/real_JPG/sparse/0/cameras.bin')
        H = camdata[1].height
        W = camdata[1].width
        self.focal = camdata[1].params[0] * self.img_wh[0] / W

        # --- Step 2: read images.bin and poses ---
        imdata = read_images_binary('C:/Users/USER/Desktop/real_JPG/sparse/0/images.bin')
        ...
        pts3d = read_points3d_binary('C:/Users/USER/Desktop/real_JPG/sparse/0/points3D.bin')
        n_images = len(poses)
        n_points = len(pts3d)
        ...
        for depth_path in tqdm(nerf_depths):
            ...
            if i == 0:
                ...
                image_files = [f for f in image_files if f.endswith('.png') or f.endswith('.jpg') or f.endswith('.JPG')]
...
for scene in ['plante']:
    print(scene)
    root_dir = f'/content/drive/MyDrive/NeRF-SR/nerf_llff_data/{scene}'
    result_dir = f'/content/drive/MyDrive/NeRF-SR/checkpoints/nerf-sr/llff-{scene}-{height}x{width}-ni64-dp-ds2/30_test_vis'
