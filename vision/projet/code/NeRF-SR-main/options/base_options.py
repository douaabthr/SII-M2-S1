...
# done : ajouter les arg que je veux lire lie au n'importe qul 
...
# done
    def gather_options(self):
        ...
        #  lire les options de la ligne de commonde deja initialiser par train option et base option
        opt, _ = parser.parse_known_args()

        # modify model-related parser options
        model_name = opt.model
        # get option setter il va recuper tout les new options lie au model qui herite classe confugurable 
        #  et je peux redifinir la classe modify_commandline_options 
...
# done
    def print_options(self, opt):
        ...
        # save to the disk into forma txt and json in checkpoint dirctori
...
#  done
    def parse(self, rank):
        ...
        #  return objet self lire parser de la ligne de commande et redfinir or add additionerl option
        ...
        # process opt.suffix 
        # ce suffix par defait vide et name = llff_-fern-378x504-ni64-dp-ds2
        if opt.suffix:
            suffix = ('_' + opt.suffix.format(**vars(opt))) if opt.suffix != '' else ''
            opt.name = opt.name + suffix

        # ddp 
        #  verfier sil sagit de ddp et not traning exote car support only training 
        if opt.accelerator == 'ddp':
            if not opt.isTrain:
                print('Distributed Data Parallel is only supported in training.')
                exit(0)
            opt.local_rank = rank
        # ce code exeucter par a master gpu cad une seul gpu si ya pas de gpu alors local rank=0 
        opt.is_master = True if opt.accelerator == 'dp' else opt.local_rank == 0
        #  count numbre of gpu et initalier opt.device par cude: rank
        opt.n_gpus = torch.cuda.device_count()
        # initaialiser opt.device par rank of gpus
